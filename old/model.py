...
class TransformerSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, layer_past, mask):
        ...
        attention_scores.masked_fill_(1 - mask.byte(), -6e4)
...
class TransformerEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, past_length, unidirectional, token_type_ids=None):
        position_ids = torch.arange(
            past_length, input_ids.shape[-1] + past_length, dtype=torch.long, device=input_ids.device)
        # FIX: potential problem here
...
class TransformerModel(nn.Module):
    def __init__(self, unidirectional):
        super().__init__()
        self.embeddings = TransformerEmbeddings()
        self.encoder = TransformerEncoder()
        self.unidirectional = unidirectional

    def forward(self, input_ids, mask, token_type_ids=None, past=None, past_length=None):
        """
        ...
        embedding_output = self.embeddings(input_ids, past_length, self.unidirectional, token_type_ids)
...
class TransformerLMHeadModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.transformer = TransformerModel(unidirectional=True)
        self.projection = nn.Linear(768, 21128, bias=False)

        self.set_tied()

    def set_tied(self):
        """Although weights are set tied explicitly here, when loading weights,
        call it again.
        """
    ...
    def forward(self, input_ids, mask, past=None, past_length=None):
        hidden_states, presents = self.transformer(input_ids, mask, past=past, past_length=past_length)
